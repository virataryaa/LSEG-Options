...
A sidebar "View" switch adds Px Change, Vol Surface, and IV vs RV (ported
from the former standalone oi_advanced_analytics.py) — only loading futures
data and computing those panels when Advanced Analytics is actually
selected, so the default Basic view is unaffected in speed.
...
view_mode = c.render_view_toggle()
...
# ── Advanced Analytics tab renderer — Px Change / Vol Surface / IV vs RV ───────
# Ported verbatim from the former standalone oi_advanced_analytics.py (only
# renamed and given a lazy plotly import) — only called when the sidebar
# "View" switch is set to Advanced Analytics, so Basic mode never pays for
# this file existing.
def render_advanced_tab(df, atm_val, atm_label, old_date, new_date,
                        key_prefix, title, display_step=None, mround_default=None,
                        ingest_note="", fut_df=None):
    import plotly.graph_objects as go  # lazy — only Advanced mode needs plotly here
...
if view_mode == "Advanced Analytics":
    # Futures data + Px Change/Vol Surface/IV vs RV only load/compute here —
    # Basic mode (the default) never touches this branch at all.
